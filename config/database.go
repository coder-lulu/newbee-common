...
func (c DatabaseConf) MysqlDSN() string {
	// 🔥 确保MysqlConfig以&开头，避免DSN格式错误
	mysqlConfig := c.MysqlConfig
	if mysqlConfig != "" && mysqlConfig[0] != '&' {
		mysqlConfig = "&" + mysqlConfig
	}
	return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?parseTime=True%s", c.Username, c.Password, c.Host, c.Port, c.DBName, mysqlConfig)
}
