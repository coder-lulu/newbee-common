...
func (p *AuditPlugin) Init(core *framework.CoreServices) error {
	...
	// 处理配置：优先使用内置配置，其次使用核心服务配置
	if p.builtinConfig != nil {
...
func (p *AuditPlugin) Handle(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if p.shouldSkip(r.URL.Path) {
			logger.WithField("skipped", true).Debug("audit logging skipped for path")
			next(w, r)
			return
		}
		...
		// 3. 处理响应体
		responseBody := ""
		if responseBuffer.Len() > 0 && p.shouldCaptureResponseBody(r) {
		...
		if p.builtinWriter != nil || p.asyncQueue != nil {
			...
			auditData := framework.AuditLogData{
				TenantID:     tenantID,
				UserID:       userID,
				UserName:     "", // Will be resolved by the writer implementation
				Method:       r.Method,
				Path:         r.URL.Path,
