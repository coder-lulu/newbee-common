...
const (
	...
	OriginalTenantIDKey ContextKey = "originalTenantId" // 记录原始租户（动态切换前）
	...
	DataPermContextKey ContextKey = "dataPermContext" // 数据权限上下文
	AuditMetadataKey   ContextKey = "auditMetadata"   // 审计附加标记
...
func GetContextKey(name string) ContextKey {
	...
	case "tenant", "tenantId", "tenant-id":
		return TenantIDKey
	case "originalTenantId", "original-tenant-id":
		return OriginalTenantIDKey
	...
	case "dataScope":
		return DataScopeKey
	case "dataPermContext":
		return DataPermContextKey
	case "auditMetadata":
		return AuditMetadataKey
	case "nickname":
		return NicknameKey
	case "avatar":
		return AvatarKey
...
func IsStandardKey(key string) bool {
	standardKeys := []string{
		string(TenantIDKey), string(OriginalTenantIDKey), string(UserIDKey), string(UsernameKey),
		string(RoleCodesKey), string(DeptIDKey), string(DataScopeKey), string(DataPermContextKey),
		string(AuditMetadataKey), string(NicknameKey), string(AvatarKey), string(RequestIDKey), string(TraceIDKey),
