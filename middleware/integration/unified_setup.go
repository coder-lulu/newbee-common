...
import (
	...
	"github.com/coder-lulu/newbee-common/middleware/encryption"
...
type Config struct {
	...
	TenantInfoProvider  framework.TenantInfoProvider  `json:"-"` // 租户信息提供者 (运行时注入)

	// RBAC（Casbin）提供者（运行时注入），用于权限校验插件
	RbacProvider permission.EnforcerProvider `json:"-"`

	// 数据权限专用的Casbin提供者（运行时注入），用于统一数据权限插件
	DataPermCasbinProvider dataperm.CasbinProvider `json:"-"`
}
...
func Setup(config *Config) (*Result, error) {
	...
	// 注入租户信息提供器（需要在插件初始化之前完成）
	manager.GetCoreServices().TenantProvider = config.TenantInfoProvider
...
func registerPlugins(manager *framework.MiddlewareManager, middlewareConfig *framework.UnifiedConfig, config *Config) error {
	...
	for _, pluginName := range actualEnabledPlugins {
		...
		case "rbac":
			if config.RbacProvider != nil {
				plugins = append(plugins, permission.NewRbacPluginWithProvider(config.RbacProvider))
			} else {
				plugins = append(plugins, permission.NewRbacPlugin())
			}
		case "dataperm":
			...
			if config.DataPermCasbinProvider != nil {
				// 优先使用专用的DataPermCasbinProvider
				casbinProvider = config.DataPermCasbinProvider
				logx.Info("✅ Using dedicated DataPermCasbinProvider")
			} else if config.RbacProvider != nil {
				// 尝试从RbacProvider转换（通常Core RPC通过RpcCasbinRuleQuerier实现了CasbinProvider接口）
				if cp, ok := config.RbacProvider.(dataperm.CasbinProvider); ok {
					casbinProvider = cp
					logx.Info("✅ Using RbacProvider as CasbinProvider")
				} else {
					logx.Info("RbacProvider does not implement CasbinProvider interface",
						logx.Field("type", fmt.Sprintf("%T", config.RbacProvider)))
				}
			}

			if casbinProvider == nil {
				// 没有CasbinProvider，返回错误（强制要求）
				return fmt.Errorf("dataperm plugin requires CasbinProvider - please provide config.DataPermCasbinProvider or config.RbacProvider")
			}
		...
		case "encryption":
			plugins = append(plugins, encryption.NewEncryptionPlugin())
		default:
...
func filterByPluginConfig(plugins []string, config *framework.UnifiedConfig) []string {
	...
	for _, plugin := range plugins {
		...
		case "rbac":
			if config.Permission != nil && config.Permission.Enabled {
				filtered = append(filtered, plugin)
			}
		case "dataperm":
...
func getEnabledPlugins(plugins, skipPlugins []string) []string {
	allPlugins := []string{"auth", "tenant", "rbac", "dataperm", "audit", "encryption"}
