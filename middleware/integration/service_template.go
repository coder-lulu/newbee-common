...
func NewServiceContext(c config.Config) *ServiceContext {
	...
	// 2. 配置中间件集成
	setupConfig := &integration.QuickSetupConfig{
		MiddlewareConfig: &c.Middleware,
		RedisClient:      rds,
		I18nConfig:       &c.I18nConf,
		I18nFS:           i18n2.LocaleFS,
		// 可选配置
	...
	// 3. 快速集成中间件
	result, err := integration.QuickSetup(setupConfig)
	if err != nil {
	...
	return &ServiceContext{
		...
		Trans:                  result.Translator,
		ManagedMiddlewareChain: result.ManagedMiddlewareChain,
		// 初始化服务特定依赖
		// Redis:   rds,
...
func main() {
	...
	// 应用统一中间件链（全局模式）
	integration.ApplyToServer(server, ctx.ManagedMiddlewareChain, "global")
