...
func (l *MiddlewareLogger) WithError(err error) *MiddlewareLogger {
	if err != nil {
		...
		l.fields["error_type"] = fmt.Sprintf("%T", err)
		
		// 获取调用栈信息
		if pc, file, line, ok := runtime.Caller(1); ok {
			l.fields["caller_file"] = file
			l.fields["caller_line"] = line
			if fn := runtime.FuncForPC(pc); fn != nil {
				l.fields["caller_func"] = fn.Name()
			}
		}
	}
	return l
...
func (l *MiddlewareLogger) WithDuration(start time.Time) *MiddlewareLogger {
	...
	l.fields["duration_ms"] = float64(duration.Nanoseconds()) / 1e6
	return l
}
