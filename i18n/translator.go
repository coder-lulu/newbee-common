...
// JSONError 实现了能被httpx.ErrorCtx正确处理为JSON响应的错误类型
...
func (e *JSONError) Error() string {
	// 返回JSON字符串，go-zero会直接使用这个作为响应体
	data, _ := json.Marshal(e)
	return string(data)
}

// WriteResponse 实现 httpx 的错误响应接口
func (e *JSONError) WriteResponse(w http.ResponseWriter) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.WriteHeader(http.StatusOK) // 业务错误通常返回200状态码
