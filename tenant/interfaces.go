...
// CompletionAwarePlugin 支持初始化完成钩子的插件需要实现该接口
type CompletionAwarePlugin interface {
	// OnInitializationComplete 在所有插件成功执行后触发，可用于发送通知或刷新缓存
	OnInitializationComplete(ctx context.Context, req *InitRequest, result *ExecutionResult) error
}
