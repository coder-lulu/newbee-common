...
func (pm *PluginManager) ExecutePlugins(ctx context.Context, req *InitRequest) (*ExecutionResult, error) {
	...
	if result.Status == StatusSuccess {
		pm.notifyCompletionHooks(ctx, plugins, req, result)
	}
...
func (pm *PluginManager) notifyCompletionHooks(ctx context.Context, orderedPlugins []TenantInitPlugin, req *InitRequest, result *ExecutionResult) {
	for _, plugin := range orderedPlugins {
		completionAware, ok := plugin.(CompletionAwarePlugin)
		if !ok {
			continue
		}

		pluginName := plugin.GetMetadata().Name
		if err := completionAware.OnInitializationComplete(ctx, req, result); err != nil {
			pm.logger.Errorw("Tenant initialization completion hook failed",
				logx.Field("plugin", pluginName),
				logx.Field("tenant_id", req.TenantID),
				logx.Field("error", err.Error()))
		} else {
			pm.logger.Infow("Tenant initialization completion hook executed",
				logx.Field("plugin", pluginName),
				logx.Field("tenant_id", req.TenantID))
		}
	}
